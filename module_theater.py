...
# Speech -----------------------------
  def speech(self, id, sce, i):
    self.stop_event = threading.Event()
    self.thread = threading.Thread(target = self.speechTalk, kwargs={'sce':sce, 'i':i})
    self.thread.start()
